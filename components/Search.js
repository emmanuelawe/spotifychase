...
const Search = ({search, setSearch}) => {
  return (
    ...
    <input type='search' value={search} onChange={(e) => setSearch(e.target.value)}
     placeholder='Search...' className='bg-[#1a1a1a] text-white border-none lg:w-full 
     w-full focus:ring-0 outline-none placeholder-[#fafafa] text-xs' />

     <div className='flex items-center divide-dotted divide-x-2 divide-[#333] ml-auto'>
        <div className='flex space-x-2 pr-5'>
            <button className='tag'>Afrobeats</button>
            <button className='tag'>R&B</button>
            <button className='tag'>Alte</button>
        </div>
